...
export function Gallery() {
  ...
  const loadGallery = async () => {
    try {
      ...
      // Try to load from Firebase as well, but prioritize JSON
      try {
        ...
        console.warn('Firebase gallery loading failed, using JSON only:', firebaseError);
      }
                    ...
                    <div className="relative aspect-[4/3] overflow-hidden">
                      {item.image.endsWith('.mp4') ? (
                        <video
                          ...
                          loop
                          onError={(e) => {
                            console.error('Video failed to load:', item.image);
